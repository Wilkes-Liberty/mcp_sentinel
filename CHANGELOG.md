...
- #3624538: Composer requires `drupal/tool` `^1.0.0-beta8` instead of `*`.
  ...
  could install an older Tool API that does not satisfy it.
- #3624538: `mcp_sentinel_server` depends on `mcp_server_tool_bridge`
  2.0.0-beta3 or later. A stale bridge from before the plugin-directory move
  answers HTTP 200 with zero tools. The test dependency on
  `drupal/mcp_server` uses the same floor.
